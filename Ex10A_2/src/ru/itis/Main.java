...
public class Main1 {
    ...
    public static void main(String[] args) {
        ...
        int x = sc.nextInt(); // непонятно, почему х целый?
        ...
        for(int k = 1; k <= n; k++){
            ...
            x0 *= (-1.0)*x*x*(2.0*(k-1.0)+1.0)*(1.0/k)*(1.0/(2.0*k + 1.0)); //Да, что-то в этом роде
            // я бы записала короче: x0 *= (-1.0)*x*x*(2*(k-1)+1)/(k*(2*k + 1));
